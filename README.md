...
compiled with Sphinx for HTML / PDF viewing. The current documentation has been compiled with
...
### Version 3.3 (June 2025)

- Updated this README file.
- Added PGA calculation to `qt.TimeHistory.differentiate` when a velocity time history is
  differentiated.
- Updated the docstring of `qt.response_spectrum._solode`.
- The `qt.calcrs` function now accepts displacement time histories as input.
- To support the calculation of response spectra based on displacement base motion, a new internal
  function, `qt._vsolode`, has been created.
- Added arguments `label` and `fmt` to function `qt.envelope`.
- Added support for plot titles in `qt.plotrs`, `qt.plotps`, and `qt.plotth`.
- Added a default grid to plots generated by `qt.plotth`, with option for choosing other grids.
- The `qt.loadth` now accepts path-like objects as its first argument (restricted to strings in
  previous versions).
...
License
-------
Copyright (C) 2020-2025 Andreas H. Nielsen
